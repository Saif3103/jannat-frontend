        ...
        <div className="flex items-center justify-center gap-4 mb-10">
          {steps.map((s, i) => (
            <div key={i} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${step > i + 1 ? 'bg-emerald-500 text-white' : step === i + 1 ? 'bg-amber-500 text-black' : 'border border-amber-900/30 text-amber-100/30'}`}>
                {step > i + 1 ? <FiCheck size={14} /> : i + 1}
              </div>
              <span className={`text-sm hidden md:block ${step === i + 1 ? 'text-amber-400' : 'text-amber-100/30'}`}>{s}</span>
          ...
          <div className="lg:col-span-2">
            ...
            {step === 1 && (
              <motion.form onSubmit={handleAddressSubmit} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card p-6 space-y-4">
                <h2 className="font-luxury text-2xl text-amber-400 mb-4">Shipping Address</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={f.key} className={f.full ? 'sm:col-span-2' : ''}>
                      <label className="text-xs text-amber-100/50 block mb-1.5 tracking-wider uppercase">{f.label}</label>
            ...
            {step === 2 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card p-6">
                <h2 className="font-luxury text-2xl text-amber-400 mb-6">Payment Method</h2>
                  ...
                  {PAYMENT_METHODS.map(m => (
                    <button key={m.id} onClick={() => setPaymentMethod(m.id)}
                      ...
                      <div className="flex-1">
                        <p className={`font-medium ${paymentMethod === m.id ? 'text-amber-400' : 'text-amber-100/80'}`}>{m.label}</p>
                        <p className="text-xs text-amber-100/40">{m.desc}</p>
                      </div>
            ...
            {step === 3 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                <div className="glass-card p-5">
                  <h3 className="text-amber-400 font-medium mb-3">Shipping To</h3>
                  <p className="text-amber-100/70 text-sm">{address.name} • {address.phone}</p>
                  <p className="text-amber-100/50 text-sm">{address.street}, {address.city}, {address.state} - {address.pincode}</p>
                </div>
                <div className="glass-card p-5">
                  <h3 className="text-amber-400 font-medium mb-3">Payment</h3>
                  <p className="text-amber-100/70 text-sm">{PAYMENT_METHODS.find(m => m.id === paymentMethod)?.label}</p>
                </div>
              ...
              {items.map(item => (
                <div key={`${item._id}-${item.size}`} className="flex gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-amber-100 text-sm truncate">{item.name}</p>
                    <p className="text-amber-100/40 text-xs">Qty: {item.quantity}</p>
                    <p className="text-amber-400 text-sm font-medium">₹{((item.discountPrice || item.price) * item.quantity).toLocaleString()}</p>
                  </div>
                </div>
              ))}
            </div>
            <div className="border-t border-amber-900/20 pt-4 space-y-2">
              <div className="flex justify-between text-sm"><span className="text-amber-100/50">Subtotal</span><span className="text-amber-100">₹{subtotal.toLocaleString()}</span></div>
              <div className="flex justify-between text-sm"><span className="text-amber-100/50">Shipping</span><span className={shipping === 0 ? 'text-emerald-400' : 'text-amber-100'}>{shipping === 0 ? 'FREE' : `₹${shipping}`}</span></div>
              <div className="flex justify-between text-sm"><span className="text-amber-100/50">Tax</span><span className="text-amber-100">₹{tax.toLocaleString()}</span></div>
              <div className="flex justify-between pt-2 border-t border-amber-900/20">
                <span className="font-luxury text-amber-100">Total</span>
                <span className="font-luxury text-xl text-amber-400">₹{total.toLocaleString()}</span>
              </div>
            </div>
          </div>
        </div>
