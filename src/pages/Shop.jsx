      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-12">
        ...
        <div className="mb-12">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <p className="text-[#1A1A1A] text-xs font-bold uppercase tracking-[0.4em] mb-4">Our Collection</p>
            ...
            <div className="flex items-center gap-4">
              <div className="hidden sm:block text-right">
                <p className="text-[10px] font-bold text-[#1A1A1A]/30 uppercase tracking-widest mb-1">Available Pieces</p>
        ...
        <div className="flex flex-wrap items-center gap-4 mb-10 pb-8 border-b border-amber-900/10">
          <button 
            ...
            className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-bold text-xs transition-all ${showFilters ? 'bg-[#C9A84C] text-black' : 'bg-white/5 text-[#1A1A1A]/60 hover:bg-white/10'}`}
          >
            <FiFilter size={16} /> 
          ...
          <div className="flex flex-wrap gap-2 ml-auto">
             {category && (
               <div className="bg-[#C9A84C]/10 text-[#1A1A1A] px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider flex items-center gap-2 border border-[#C9A84C]/20">
                  ...
                  <FilterSection title="Price Range">
                    <div className="flex gap-3 pt-2">
                      <div className="relative flex-1">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#1A1A1A]/20 text-[10px] font-bold">₹</span>
                        <input 
                          type="number" 
                          placeholder="Min" 
                          value={minPrice} 
  ...
  return (
    <div className="border-b border-amber-900/20 pb-5">
      <button onClick={() => setOpen(!open)} className="flex items-center justify-between w-full mb-3">
        <h3 className="text-[#1A1A1A] text-xs tracking-widest uppercase font-medium">{title}</h3>
        <motion.span animate={{ rotate: open ? 180 : 0 }}><FiChevronDown size={14} className="text-[#1A1A1A]/40" /></motion.span>
      </button>
